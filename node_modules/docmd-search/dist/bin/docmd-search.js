...
async function saveBatch(outDir2, batchId, chunks, vectors, dimensions, compression) {
  const batchDir = join4(outDir2, "batches");
...
async function createSearchIndex(index, outDir2, options) {
  await mkdir2(outDir2, { recursive: true });
  const compression = getCompressionType(index.chunks.length);
  await saveBatch(outDir2, 0, index.chunks, index.vectors, index.dimensions, compression);
  ...
  await saveManifest(outDir2, manifest);
  if (!options?.silent) {
    ...
    console.log(`  Index written \u2192 ${outDir2}`);
...
// src/bin/docmd-search.ts
import { resolve as resolve2, join as join8 } from "node:path";
...
async function saveGlobalConfig(config2) {
  ...
  await writeFile(GLOBAL_CONFIG_FILE, JSON.stringify(config2, null, 2), "utf-8");
}
async function isWizardCompleted() {
  const config2 = await loadGlobalConfig();
  return config2?.wizardCompleted === true;
}
...
async function resolveConfig(rootDir, overrides) {
  ...
  if (global?.model) {
    config2.model = global.model;
  }
  if (rootDir) {
    const project = await loadProjectConfig(rootDir);
    if (project) {
      Object.assign(config2, project);
    }
  }
  if (overrides) {
...
function createModelManager(modelId, onProgress) {
  ...
  let pipeline = null;
  let loaded = false;
  const notify = (phase, message, progress2) => {
    onProgress?.({ phase, message, progress: progress2 });
  };
  return {
    profile,
    async load() {
      ...
      try {
        ...
        env.cacheDir = join9(homedir2(), ".docmd-search", "models");
...
async function indexDirectory(options, onProgress) {
  const {
    ...
    chunkOverlap,
    outDir: outDirName,
    model: modelOverride2,
    config: configOverride
  } = options;
  const config2 = await resolveConfig(rootDir, {
    ...configOverride,
    ...include ? { include } : {},
    ...exclude ? { exclude } : {},
    ...
    ...modelOverride2 ? { model: modelOverride2 } : {}
  });
  const outDir2 = isAbsolute(config2.outDir) ? config2.outDir : join6(rootDir, config2.outDir);
  const modelProfile = getModelProfile(config2.model);
  ...
  const filePaths = await crawl(rootDir, config2.include, config2.exclude);
  notify(
    "crawling",
    filePaths.length,
    filePaths.length,
  ...
  const existingManifest = await loadManifest(outDir2);
  let filesToIndex = filePaths;
  if (config2.incremental && existingManifest) {
    ...
    if (changed.length === 0 && existingManifest.status === "complete") {
      ...
      return loadAllBatches2(outDir2);
    }
  ...
  let allVectors = [];
  try {
    modelManager = await getCachedModel(config2.model, (p) => {
    ...
    for (let i = 0; i < chunks.length; i += CHUNKS_PER_BATCH) {
      ...
      await saveBatch(outDir2, batchId, batchChunks, batchVectors, modelProfile.dimensions, compression);
    ...
    if (chunks.length > 0 && allVectors.length === 0) {
      ...
      await saveBatch(outDir2, 0, chunks, zeroVectors, modelProfile.dimensions, "none");
      manifest.batchCount = 1;
...
Index saved with ${manifest.totalChunks} chunks (without embeddings).`);
  ...
  await saveNavigation(outDir2, buildNavTree(navEntries));
  notify(
    "complete",
    chunks.length,
    chunks.length,
...
function printBanner(version2) {
  console.log(BANNER);
  if (version2) {
    console.log(`   ${A.dim}v${version2} \xB7 offline semantic search${A.reset}`);
    console.log("");
  }
}
...
function createProgressDisplay(verbose = false) {
  ...
  return {
    render(progress2) {
      ...
      const elapsed2 = ((Date.now() - startTime2) / 1e3).toFixed(1);
      const pct = progress2.total > 0 ? Math.round(progress2.current / progress2.total * 100) : 0;
      const bar = progressBar(progress2.current, progress2.total);
      const label = phaseLabel(progress2.phase);
      let lines = [];
      if (progress2.phase === "complete") {
        lines.push(`   ${A.green}\u2713${A.reset} ${progress2.message ?? "Complete"}  ${A.dim}(${elapsed2}s)${A.reset}`);
      } else {
        lines.push(`   ${A.magenta}${label}${A.reset}  ${bar}  ${A.bold}${pct}%${A.reset}  ${A.dim}${progress2.current}/${progress2.total}${A.reset}`);
        if (progress2.file && verbose) {
          lines.push(`   ${A.dim}\u2192 ${progress2.file}${A.reset}`);
        }
        if (progress2.message && verbose) {
          lines.push(`   ${A.dim}${progress2.message}${A.reset}`);
        }
        lines.push(`   ${A.dim}elapsed: ${elapsed2}s${A.reset}`);
      }
...
async function runSetupWizard() {
  ...
  const answer = await new Promise((resolve3) => {
    rl.question(`   ${A.magenta}Pick a model${A.reset} [1-${AVAILABLE_MODELS.length}] (default: 1): `, resolve3);
  });
  rl.close();
...
async function runSettingsTUI() {
  ...
  const answer = await new Promise((resolve3) => {
    rl.question(`   ${A.magenta}Switch model?${A.reset} [1-${AVAILABLE_MODELS.length}] (Enter to keep current): `, resolve3);
  });
  rl.close();
  const choice = parseInt(answer.trim(), 10);
...
`;
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    if (resolvedBinary) {
      ...
      args2 = ["dev", "--config", join7(stagingDir, "docmd.config.json")];
      if (verbose) {
        console.log(`   ${A2.dim}using docmd at: ${resolvedBinary}${A2.reset}`);
      }
    } else {
      command = "npx";
      args2 = ["-y", "@docmd/core", "dev", "--config", join7(stagingDir, "docmd.config.json")];
      if (verbose) {
        console.log(`   ${A2.dim}falling back to: npx @docmd/core${A2.reset}`);
      }
    }
    if (port > 0) {
      args2.push("--port", String(port));
    }
...
function getVersion() {
  try {
    const __dir = dirname2(fileURLToPath2(import.meta.url));
    const pkg = JSON.parse(readFileSync(join8(__dir, "../../package.json"), "utf-8"));
...
var showHelp = flags.has("--help") || flags.has("-h");
var showVersion = flags.has("--version") || flags.has("-v");
var launchUI = flags.has("--ui");
var isDev = flags.has("--dev");
var openSettings = flags.has("--settings");
var modelOverride;
...
if (showHelp) {
  printBanner(version);
  console.log(`   ${A3.bold}Usage${A3.reset}`);
  ...
  console.log("");
  console.log(`   ${A3.bold}Options${A3.reset}`);
  ...
  console.log("");
  console.log(`   ${A3.bold}Examples${A3.reset}`);
  console.log(`     ${A3.dim}$${A3.reset} docmd-search ./docs`);
  console.log(`     ${A3.dim}$${A3.reset} docmd-search ./docs --ui`);
  console.log(`     ${A3.dim}$${A3.reset} docmd-search --settings`);
  console.log(`     ${A3.dim}$${A3.reset} docmd-search ./my-project --model Xenova/bge-small-en-v1.5`);
  console.log("");
  process.exit(0);
}
if (openSettings) {
...
var wizardDone = await isWizardCompleted();
if (!wizardDone) {
  clearScreen();
  printBanner(version);
  await runSetupWizard();
}
var missingDeps = checkPeerDeps();
if (missingDeps) {
  console.log(formatMissingDepsMessage(missingDeps.missing));
  console.log(`   ${A3.dim}These packages are needed for generating embeddings.${A3.reset}`);
  console.log(`   ${A3.dim}After installing, run docmd-search again.${A3.reset}`);
  console.log("");
  process.exit(1);
}
var targetDir = resolve2(positional[0] ?? ".");
var config = await resolveConfig(targetDir, modelOverride ? { model: modelOverride } : void 0);
var outDir = join8(targetDir, config.outDir);
clearScreen();
printBanner(version);
await printEngineStatus();
if (isDev) {
  console.log(`   ${A3.dim}directory  ${targetDir}${A3.reset}`);
  console.log(`   ${A3.dim}index      ${outDir}${A3.reset}`);
  console.log(`   ${A3.dim}model      ${config.model}${A3.reset}`);
  console.log("");
}
var progress = createProgressDisplay(isDev);
...
try {
  ...
  progress.clear();
  console.error(`   ${A3.red}\u2717${A3.reset} ${err.message}`);
  console.error("");
  if (hasSearchableIndex(outDir)) {
    console.log(`   ${A3.yellow}Using partial index...${A3.reset}`);
...
progress.clear();
var elapsed = ((performance.now() - startTime) / 1e3).toFixed(2);
var fileCount = new Set(searchIndex.chunks.map((c) => c.file)).size;
console.log(`   ${A3.green}\u2713${A3.reset} ${searchIndex.chunks.length} chunks from ${fileCount} files ${A3.dim}(${elapsed}s)${A3.reset}`);
console.log("");
if (launchUI) {
  console.log(`   ${A3.magenta}\u25C6${A3.reset} Launching web UI via docmd...`);
  console.log("");
  try {
...
`);
    ...
    console.log(`   ${A3.dim}Falling back to TUI search...${A3.reset}`);
    console.log("");
  }
}
function performSearch(idx, query, topK = 10) {
 ...
 * --------------------------------------------------------------------
 */
//# sourceMappingURL=docmd-search.js.map
