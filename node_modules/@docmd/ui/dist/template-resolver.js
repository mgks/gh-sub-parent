...
function readDefaultTemplate(slot) {
...
function matchesAny(input, globs) {
    if (!globs || globs.length === 0)
        return true;
    for (const g of globs) {
        if (globToRegExp(g).test(input))
            return true;
    }
    return false;
}
function matchesNone(input, globs) {
    if (!globs || globs.length === 0)
        return true;
    for (const g of globs) {
        if (globToRegExp(g).test(input))
...
function normaliseTemplateRef(ref) {
    if (!ref)
        return null;
    if (typeof ref === 'string')
        return { name: ref };
    if (typeof ref === 'object' && typeof ref.name === 'string')
        return ref;
    return null;
}
// ---------------------------------------------------------------------------
...
export function resolveTemplate(ctx) {
    ...
    if (!defaultPath) {
        ...
        throw new Error(`[docmd/template] No default template found for slot "${type}". ` +
...
// Internals
// ---------------------------------------------------------------------------
function matchSectionTemplate(templates, pagePath) {
    if (!templates || typeof templates !== 'object')
        return null;
    for (const [pattern, ref] of Object.entries(templates)) {
        if (!globToRegExp(pattern).test(pagePath))
            continue;
...
function findPluginTemplate(type, name, pagePath) {
    ...
    const matches = hooks.templates
        ...
        .filter((t) => {
        const pluginSlug = String(t._pluginName || '')
            .replace(/^@docmd\//, '')
            .replace(/^template-/, '');
