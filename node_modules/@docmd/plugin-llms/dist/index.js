...
export async function onPostBuild({ config, pages, outputDir, log }) {
    ...
    if (log)
        log('Generating LLMs context files' + (i18n && localeIds.length > 1 ? ' (multi-locale)' : ''));
     ...
     */
    function pageLocale(p) {
        const parts = String(p.outputPath || '').split('/').filter(Boolean);
        if (localeIds.length && parts.length && localeIds.includes(parts[0]))
            return parts[0];
        return defaultLocale;
    }
    const grouped = new Map();
    for (const p of pages) {
        if (p.frontmatter.noindex)
            continue;
        if (p.frontmatter.llms === false)
            continue;
    ...
    for (const loc of localesToWrite) {
        ...
        const safeForMarkdownAndCsv = (raw) => {
        ...
        if (config.description)
            content += `${safeForMarkdownAndCsv(config.description)}\n\n`;
        ...
        if (config.description)
            fullContent += `${safeForMarkdownAndCsv(config.description)}\n\n`;
        fullContent += `---\n\n`;
        for (const page of bucket) {
            const pathname = outputPathToPathname(page.outputPath);
            const fullUrl = sanitizeUrl(siteUrl + pathname);
            ...
            try {
                ...
                if (log)
                    log(`Skipping raw markdown: ${page.sourcePath}`, 'SKIP');
            }
        }
        ...
        await fs.writeFile(path.join(outputDir, `${jsonBase}.json`), JSON.stringify(llmsJson, null, 2));
    }
}
