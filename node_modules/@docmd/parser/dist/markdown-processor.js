...
function emitNormaliserWarning(w) {
    normaliserWarnings.push(w);
    if (normaliserVerbose) {
        console.warn(`[normaliser] ${w.severity.toUpperCase()} ${w.path}:${w.line} — ${w.message}`);
    }
}
export function setNormaliserVerbose(verbose) {
    normaliserVerbose = verbose;
}
export function flushNormaliserWarnings() {
    if (normaliserWarnings.length === 0)
        return;
    ...
    const reset = '\x1b[0m';
    console.warn(`${tag}[normaliser] ${severity}${reset} ` +
 ...
 */
function applyContainerNormaliser(markdownContent, env) {
...
const headingIdPlugin = (md, options = {}) => {
...
async function processContentAsync(rawString, mdInstance, config, env = {}, hooks = null) {
    let frontmatter, markdownContent;
    try {
        const parsed = matter(rawString);
        frontmatter = parsed.data;
