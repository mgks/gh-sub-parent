...
export function resolveHref(href) {
    if (!href)
        return { href, isExternal: false, isRaw: false };
    ...
    if (href.match(/^(?:[a-z][a-z0-9+.-]*:|\/\/)/i) || // http:, https:, mailto:, tel:, ftp:, //
        href.startsWith('#') ||
        href.match(/(^|\/)assets\//)) {
 ...
 */
export function normalizeNavPaths(items) {
    if (!items)
        return;
 ...
 */
export function normalizeMenubarPaths(items) {
    if (!items)
        return;
    for (const item of items) {
