...
function tagInlineRule(state, silent) {
    ...
    const match = state.src.slice(start, max).match(/^:::\s*tag\s+(?:["']([^"']+)["']|(\S+))((?:\s+(?:icon|color|link|url):(?:"[^"]*"|'[^']*'|\S+))*)/);
    if (!match)
        return false;
    if (silent)
        return true;
    ...
    for (const part of parts) {
        ...
        if (part.startsWith('icon:'))
            icon = unquote(part.substring(5));
        else if (part.startsWith('color:'))
            color = unquote(part.substring(6));
        else if (part.startsWith('link:'))
            link = unquote(part.substring(5));
        else if (part.startsWith('url:'))
            link = unquote(part.substring(4));
    }
