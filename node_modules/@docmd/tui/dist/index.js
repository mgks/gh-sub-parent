...
/* ── TTY helpers ────────────────────────────────────────────── */
function isTTY() { return process.stdout.isTTY === true; }
...
export const TUI = {
    ...
    dim: chalk.dim,
    bold: chalk.bold,
    banner: (logo = LOGO, version = PKG_VERSION) => {
        commitState();
        ...
        // banner convention.
        if (process.env.NO_COLOR || process.env.DOCMD_NO_BANNER)
            return;
        console.log(`\n${chalk.blue(logo)}`);
        console.log(`${chalk.dim(` v${version}`)}\n`);
    },
    section: (label, color = chalk.cyan) => {
        commitState();
    ...
    divider: (label, color = chalk.blue) => {
        commitState();
     ...
     */
    step: (label, status = 'WAIT', barColor = chalk.cyan, _statusFirst) => {
        const f = flag(status);
    ...
    item: (label, value, labelColor = chalk.dim, barColor = chalk.cyan) => {
        commitState();
     ...
     */
    pluginTree: (pluginName, entries, color = chalk.cyan) => {
        commitState();
     ...
     *   │
     */
    progress: (label, current, total, barColor = chalk.cyan) => {
     ...
     */
    spinner: (label, barColor = chalk.cyan) => {
        let frameIndex = 0;
        ...
        const finish = (status, finalLabel) => {
            stopped = true;
            if (interval)
        ...
        return {
            ...
            done: (doneLabel, _statusFirst) => finish('DONE', doneLabel),
            fail: (failLabel, _statusFirst) => finish('FAIL', failLabel),
        };
    },
    // ── Counter ────────────────────────────────────────────────
    counter: (label, count, barColor = chalk.cyan) => {
        const line = `${barColor('│')}  ${chalk.dim(label)} ${chalk.bold(String(count))}`;
        if (isTTY())
            process.stdout.write(`\r\x1b[K${line}`);
    },
    commitLine: (label, barColor = chalk.cyan) => {
        commitState();
    ...
    projectDetails: (opts) => {
        ...
        if (opts.engine)
            TUI.item('Engine', opts.engine === 'rust' ? 'rust (preview)' : opts.engine, chalk.dim, bc);
        if (opts.source)
            TUI.item('Source', opts.source, chalk.dim, bc);
        if (opts.output)
            TUI.item('Output', opts.output, chalk.dim, bc);
        if (opts.versions)
            TUI.item('Versions', `${opts.versions.count} (${opts.versions.labels})`, chalk.dim, bc);
        if (opts.locales)
            TUI.item('Locales', `${opts.locales.count} (${opts.locales.labels})`, chalk.dim, bc);
        if (opts.threads)
            TUI.item('Threads', `${opts.threads}`, chalk.dim, bc);
    },
    /**
     * Extract standardised project details from a resolved config object.
     */
    extractProjectDetails: (config, outputDir, cwd) => {
        const details = {
        ...
        if (config.versions?.all?.length > 0) {
            details.versions = {
                ...
                labels: config.versions.all.map((v) => v.id).join(', '),
            };
        }
        if (config.i18n?.locales?.length > 0) {
            details.locales = {
                count: config.i18n.locales.length,
                labels: config.i18n.locales.map((l) => l.id).join(', '),
            };
        }
        return details;
    },
