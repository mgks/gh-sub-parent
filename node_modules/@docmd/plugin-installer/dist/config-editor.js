...
export function detectConfigFormat(configPath) {
 ...
 */
function findPluginsBlock(content) {
 ...
 *                    (e.g. \`{}\` or \`{ theme: 'dark' }\`).
 */
export function addPluginToPluginsBlock(content, configKey, valueText) {
    // Comment-aware "already configured" check.
    const stripped = stripComments(content);
    const alreadyRegex = new RegExp(`(?:^|[\\s,\\{])(['"\`]?)${escapeRe(configKey)}\\1\\s*:`, 'm');
    ...
    if (block) {
        ...
        if (trimmed.trim() === '') {
            const newInner = `\n${newEntry}\n${indent}`;
            return {
                newContent: content.slice(0, block.start + 1) + newInner + content.slice(block.end),
                changed: true
            };
        }
 ...
 */
function addPluginsBlockToTopLevel(content, configKey, valueText) {
    ...
    // nested one.
    const exportMatch = stripped.match(/(?:export\s+default\s+(?:defineConfig(?:\s*<[^>]*>)?\s*)?|module\.exports\s*=\s*(?:defineConfig\s*)?)\s*\{/);
    if (!exportMatch)
        return content;
 ...
 */
export function removePluginFromPluginsBlock(content, configKey) {
    // Comment-aware "is present" check.
    const stripped = stripComments(content);
    const presentRegex = new RegExp(`(?:^|[\\s,\\{])(['"\`]?)${escapeRe(configKey)}\\1\\s*:`, 'm');
    ...
    return {
        newContent: content.slice(0, block.start + 1) +
            newInner +
...
// ─────────────────────────────────────────────────────────────────────
export function addPluginToJsonConfig(content, configKey, valueText) {
    let config;
    try {
...
export function removePluginFromJsonConfig(content, configKey) {
    let config;
    try {
        config = JSON.parse(content);
    }
