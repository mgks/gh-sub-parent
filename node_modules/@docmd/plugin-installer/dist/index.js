...
import { detectConfigFormat, addPluginToJsonConfig, addPluginToPluginsBlock, removePluginFromJsonConfig, removePluginFromPluginsBlock } from './config-editor.js';
 ...
 */
function formatSpawnError(err, cmdExe, action, opts = {}) {
    ...
    if (isMissingBinary) {
        ...
        const nodeReachable = (() => {
            try {
                require('child_process').execFileSync(process.execPath, ['--version'], { stdio: 'pipe' });
                return true;
            }
            catch {
                return false;
    ...
    if (opts.verbose && err && err.message)
        return err.message;
 ...
 */
function resolvePackageManagerBin(name) {
...
function injectTemplateToConfig(configPath, meta) {
    ...
    if (format === 'json') {
        ...
        if (config.theme.template === templateName)
            return false;
    ...
    if (new RegExp(`template\\s*:\\s*['"\`]${escaped}['"\`]`).test(content))
        return false;
...
function removeTemplateFromConfig(configPath) {
    ...
    if (format === 'json') {
        ...
        if (!config.theme || !('template' in config.theme))
            return false;
...
async function installPlugin(pluginInput, opts = {}) {
    ...
    if (!meta.kind && isCorePlugin(meta.configKey)) {
        TUI.error('Installation Aborted (Plugin Already Present)', `Plugin "${meta.configKey}" is a core plugin that ships with @docmd/core. ` +
    ...
    let cmdArgs = [];
    if (pkgManager === 'npm') {
        cmdExe = resolvePackageManagerBin('npm');
        cmdArgs = ['install', packageName];
    }
    else if (pkgManager === 'yarn') {
        cmdExe = resolvePackageManagerBin('yarn');
        cmdArgs = ['add', packageName];
    }
    else if (pkgManager === 'pnpm') {
        cmdExe = resolvePackageManagerBin('pnpm');
        cmdArgs = ['add', packageName];
    }
    else if (pkgManager === 'bun') {
        cmdExe = resolvePackageManagerBin('bun');
        cmdArgs = ['add', packageName];
    }
...
async function removePlugin(pluginInput, opts = {}) {
    ...
    if (!meta.kind && isCorePlugin(meta.configKey)) {
        TUI.error('Removal Aborted (Core Plugin)', `Plugin "${meta.configKey}" is a core plugin that ships with @docmd/core. ` +
    ...
    if (pkgManager === 'npm') {
        cmdExe = 'npm';
        cmdArgs = ['uninstall', packageName];
    }
    else if (pkgManager === 'yarn') {
        cmdExe = 'yarn';
        cmdArgs = ['remove', packageName];
    }
    else if (pkgManager === 'pnpm') {
        cmdExe = 'pnpm';
        cmdArgs = ['remove', packageName];
