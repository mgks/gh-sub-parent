...
import { loadRuntimeRegistry, installRuntimeDep, tryLoadAfterInstall, shortKey, } from './runtime-deps.js';
 ...
 */
function hasCapabilityForHook(descriptor, hookName) {
    if (!descriptor)
        return true; // Legacy plugins without descriptors get full access
    for (const cap of descriptor.capabilities) {
        if (CAPABILITY_HOOKS[cap]?.includes(hookName))
            return true;
    }
    return false;
}
...
// Isolation wrapper (§2)
// ---------------------------------------------------------------------------
async function safeCall(hookName, pluginName, fn, ...args) {
    try {
        ...
        return (hookName === 'injectHead' || hookName === 'injectBody') ? '' : undefined;
    }
}
...
const _pluginReturnTypeWarnings = new Set();
function warnPluginReturnTypeOnce(pluginName, hookName, message) {
    const key = `${pluginName}::${hookName}`;
    if (_pluginReturnTypeWarnings.has(key))
        return;
...
function coerceStringPluginReturn(value, hookName, pluginName) {
    if (value === null || value === undefined)
        return null;
    if (typeof value === 'string')
        return value;
    warnPluginReturnTypeOnce(pluginName, hookName, `returned a non-string value (${typeof value}); expected a string. Skipping.`);
    return null;
}
...
function coerceGenerateScriptsReturn(value, target, pluginName) {
    if (value === null || value === undefined)
        return null;
    if (typeof value === 'string') {
    ...
    warnPluginReturnTypeOnce(pluginName, 'generateScripts', `returned a non-object, non-string value (${typeof value}); expected a string or { headScriptsHtml, bodyScriptsHtml }. Skipping.`);
    return null;
}
...
function coerceTranslationsReturn(value, localeId, pluginName) {
    if (value === null || value === undefined)
        return {};
    if (typeof value !== 'object' || Array.isArray(value)) {
        warnPluginReturnTypeOnce(pluginName, 'translations', `returned a non-object value for locale "${localeId}" (${typeof value}); expected Record<string, string>. Skipping.`);
        return {};
    }
    const out = {};
...
export function isCorePlugin(name) {
...
// ---------------------------------------------------------------------------
export function resolvePluginName(key) {
    if (key.includes('/'))
        return key;
...
export async function loadPlugins(config, opts) {
    ...
    const isMonorepoContext = process.env.DOCMD_TEST === 'true' ||
    ...
    for (const [name, options] of pluginMap) {
        ...
        try {
            ...
            const manifestCapabilities = (loadRuntimeRegistry()[shortNameForManifest]?.capabilities);
            try {
...
function getCapabilitySet(shortName) {
    let set = _capabilityCache.get(shortName);
    if (set)
        return set;
    const entry = loadRuntimeRegistry()[shortName];
    set = new Set(Array.isArray(entry?.capabilities) ? entry.capabilities : []);
 ...
 */
function checkManifestCapabilityDrift(shortName, descriptor, plugin) {
    const warnings = [];
    const manifestCaps = Array.from(getCapabilitySet(shortName));
    if (manifestCaps.length === 0)
        return warnings; // Third-party or unknown package; skip.
    if (!descriptor)
        return warnings; // Legacy; skip.
    const descCaps = new Set(Array.isArray(descriptor.capabilities) ? descriptor.capabilities : []);
    // Implemented hooks vs declared capabilities.
    const KNOWN_HOOKS_BY_CAP = [
        ['markdown', 'markdownSetup'],
    ...
    for (const [cap, hook] of KNOWN_HOOKS_BY_CAP) {
        if (typeof plugin[hook] === 'function' && !descCaps.has(cap) && !manifestCaps.includes(cap)) {
            warnings.push(`exports \`${hook}\` but neither the descriptor nor the manifest ` +
                `declares the "${cap}" capability. The hook will be registered.`);
        }
    }
    return warnings;
}
function registerPlugin(name, plugin, options, manifestCapabilities) {
    ...
    const shouldExecute = (pageContext) => {
        if (!pageContext || !pageContext.frontmatter)
            return true;
        const fmPlugins = pageContext.frontmatter.plugins || {};
        if (fmPlugins[shortName] === false)
            return false;
        if (fmPlugins[shortName] === true)
            return true;
        if (pageContext.frontmatter.noStyle) {
            if (options && options.noStyle !== undefined)
                return options.noStyle;
            if (plugin.noStyle !== undefined)
                return plugin.noStyle;
            return true;
        }
        return true;
    };
    ...
    if (typeof plugin.markdownSetup === 'function') {
        if (hasCapabilityForHook(descriptor, 'markdownSetup')) {
            ...
            hooks.markdownSetup.push((md) => safeCall('markdownSetup', name, fn, md, options));
        }
        else {
    ...
    if (typeof plugin.generateMetaTags === 'function') {
        if (hasCapabilityForHook(descriptor, 'generateMetaTags')) {
            ...
            hooks.injectHead.push(async (config, pageContext, root) => {
                if (!shouldExecute(pageContext))
                    return '';
    ...
    if (typeof plugin.generateScripts === 'function') {
        if (hasCapabilityForHook(descriptor, 'generateScripts')) {
            ...
            hooks.injectHead.push(async (config, pageContext) => {
                if (!shouldExecute(pageContext))
                    return '';
                const raw = await safeCall('generateScripts', name, fn, config, options, 'head');
                return coerceGenerateScriptsReturn(raw, 'head', name) || '';
            });
            hooks.injectBody.push(async (config, pageContext) => {
                if (!shouldExecute(pageContext))
                    return '';
                const raw = await safeCall('generateScripts', name, fn, config, options, 'body');
    ...
    if (typeof plugin.onPostBuild === 'function') {
        if (hasCapabilityForHook(descriptor, 'onPostBuild')) {
            ...
            wrapper._pluginName = descriptor?.name || shortName;
    ...
    if (typeof plugin.getAssets === 'function') {
        if (hasCapabilityForHook(descriptor, 'getAssets')) {
            ...
            hooks.assets.push(async () => (await safeCall('getAssets', name, fn, options)) || []);
        }
        else {
    ...
    if (Array.isArray(plugin.templates)) {
        if (hasCapabilityForHook(descriptor, 'templates')) {
            for (const tpl of plugin.templates) {
                ...
                tpl._pluginName = descriptor?.name || shortName;
    ...
    if (Array.isArray(plugin.templateAssets)) {
        if (hasCapabilityForHook(descriptor, 'templateAssets')) {
            for (const asset of plugin.templateAssets) {
    ...
    if (typeof plugin.translations === 'function') {
        if (hasCapabilityForHook(descriptor, 'translations')) {
            ...
            hooks.translations.push(async (localeId) => {
    ...
    if (typeof plugin.onConfigResolved === 'function') {
        if (hasCapabilityForHook(descriptor, 'onConfigResolved')) {
            ...
            hooks.onConfigResolved.push(async (config) => {
                try {
                    await fn(config);
                }
                catch (err) {
    ...
    if (typeof plugin.onDevServerReady === 'function') {
        if (hasCapabilityForHook(descriptor, 'onDevServerReady')) {
            ...
            hooks.onDevServerReady.push(async (server, wss) => {
                try {
                    await fn(server, wss);
                }
                catch (err) {
    ...
    if (typeof plugin.onBeforeParse === 'function') {
        if (hasCapabilityForHook(descriptor, 'onBeforeParse')) {
            ...
            hooks.onBeforeParse.push(async (src, frontmatter, filePath) => {
                try {
    ...
    if (typeof plugin.onAfterParse === 'function') {
        if (hasCapabilityForHook(descriptor, 'onAfterParse')) {
            ...
            hooks.onAfterParse.push(async (html, frontmatter, filePath) => {
                try {
    ...
    // onBeforeBuild
    if (typeof plugin.onBeforeBuild === 'function') {
        if (hasCapabilityForHook(descriptor, 'onBeforeBuild')) {
            const fn = plugin.onBeforeBuild;
            hooks.onBeforeBuild.push(async (ctx) => {
                try {
                    await fn(ctx);
                }
                catch (err) {
    ...
    if (typeof plugin.onBeforeRender === 'function') {
        if (hasCapabilityForHook(descriptor, 'onBeforeRender')) {
            const fn = plugin.onBeforeRender;
            hooks.onBeforeRender.push(async (page) => {
                try {
                    await fn(page);
                }
                catch (err) {
    ...
    if (typeof plugin.onPageReady === 'function') {
        if (hasCapabilityForHook(descriptor, 'onPageReady')) {
            ...
            hooks.onPageReady.push(async (page) => {
                try {
                    await fn(page);
                }
                catch (err) {
