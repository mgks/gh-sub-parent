...
export function _resetRuntimeRegistryCache() {
    _registry = null;
}
// ---------------------------------------------------------------------------
...
export function detectPackageManager(cwd) {
    ...
    while (dir !== path.parse(dir).root) {
        if (nativeFs.existsSync(path.join(dir, 'pnpm-lock.yaml')))
            return 'pnpm';
        if (nativeFs.existsSync(path.join(dir, 'yarn.lock')))
            return 'yarn';
        if (nativeFs.existsSync(path.join(dir, 'bun.lockb')))
            return 'bun';
        if (nativeFs.existsSync(path.join(dir, 'package-lock.json')))
            return 'npm';
 ...
 */
export function getDocmdVersion() {
 ...
 */
export function isValidRuntimeDepName(name) {
 ...
 */
function shortNameOf(packageName) {
    if (!isValidRuntimeDepName(packageName))
        return null;
    if (packageName.startsWith('@docmd/plugin-'))
        return packageName.replace('@docmd/plugin-', '');
    if (packageName.startsWith('@docmd/template-'))
        return packageName.replace('@docmd/template-', '');
    if (packageName.startsWith('@docmd/engine-'))
        return packageName.replace('@docmd/engine-', '');
    return null;
}
/**
 ...
 */
export function getBuildStatusReporter() {
    const seen = new Map();
    return {
        begin(shortName) {
 ...
 */
function buildInstallArgs(packageName, pm) {
    switch (pm) {
...
export async function installRuntimeDep(packageName) {
    ...
    if (_failedInstalls.has(packageName))
        return false;
    ...
    const versionedPackage = resolvedVersion === 'latest' ? packageName : `${packageName}@${resolvedVersion}`;
    return new Promise((resolve) => {
        ...
        child.on('error', (err) => {
            ...
            const hasProject = (() => {
                let dir = cwd;
                while (dir !== path.parse(dir).root) {
                    if (nativeFs.existsSync(path.join(dir, 'package.json')))
                        return true;
                    dir = path.dirname(dir);
                }
                return false;
            ...
            if (!hasProject && !_noPackageJsonWarned) {
                ...
                TUI.warn(`No package.json found in ${cwd} (or any parent directory). ` +
            ...
            TUI.warn(`Auto-install of ${packageName} failed: ${surface}\n  > ${hint}`);
            _failedInstalls.add(packageName);
            resolve(false);
        });
        child.on('close', (code) => {
            ...
            const hint = isTemplate
                ? `Add "${packageName}" to your package.json dependencies, then run your normal install step.`
                : `Run "docmd add ${shortName}" to install it, or add "${packageName}" to your package.json.`;
            TUI.warn(`Auto-install of ${packageName} failed (exit ${code}): ${surface || 'unknown error'}\n  > ${hint}`);
            _failedInstalls.add(packageName);
            resolve(false);
        });
    });
 ...
 */
export function shortKey(packageName) {
 ...
 */
export function manualResolvePackageEntry(packageName, startDir) {
    ...
    while (dir !== path.dirname(dir)) {
        ...
        if (nativeFs.existsSync(candidate)) {
            try {
                ...
                if (nativeFs.existsSync(distEntry))
                    return distEntry;
            }
            catch {
 ...
 */
export async function tryLoadAfterInstall(packageName, consumerCwd = process.cwd()) {
