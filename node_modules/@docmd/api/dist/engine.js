 ...
 */
export async function loadEngine(name = 'js') {
    ...
    if (name === 'js') {
        ...
        try {
            ...
            if (!isValidRuntimeDepName('@docmd/engine-js'))
                throw err;
            const installed = await installRuntimeDep('@docmd/engine-js');
            if (installed) {
                const reloaded = await tryLoadAfterInstall('@docmd/engine-js');
                if (reloaded) {
                    const { createJsEngine } = reloaded;
                    return createJsEngine();
                }
            }
            throw new Error(`[docmd] JS engine unavailable after auto-install: ${err.message}. ` +
        ...
        try {
            ...
            if (isValidRuntimeDepName('@docmd/engine-rust')) {
                ...
                if (installed) {
                    ...
                    if (reloaded) {
                        const { createRustEngine, isRustEngineAvailable } = reloaded;
            ...
            console.warn(`[docmd] Rust engine unavailable (${error.message}), falling back to JS engine.`);
            return loadEngine('js');
        }
    }
 ...
 */
export async function isEngineAvailable(name) {
    if (name === 'js')
        return true;
    if (name === 'rust') {
        try {
...
export async function getAvailableEngines() {
    const engines = ['js'];
    if (await isEngineAvailable('rust'))
        engines.push('rust');
    return engines;
}
// ---------------------------------------------------------------------------
 ...
 */
export async function runTask(engine, type, payload, timeout, trusted = false) {
    if (!trusted && !ALLOWED_TASK_TYPES.has(type)) {
        throw new Error(`Task type '${type}' is not allowed for plugins. ` +
            `Allowed types: ${[...ALLOWED_TASK_TYPES].join(', ')}`);
    }
    const result = await engine.run({ type, payload, timeout });
    if (!result.success) {
 ...
 */
export async function discoverFiles(engine, dir, extensions, exclude) {
 ...
 */
export async function readFilesBatch(engine, paths) {
    const result = await runTask(engine, 'file:readBatch', { paths });
 ...
 */
export async function getGitLog(engine, filePaths, maxCommits = 6) {
    const result = await runTask(engine, 'git:log', { filePaths, maxCommits });
    return new Map(Object.entries(result));
}
/**
 ...
 */
export async function buildSearchIndex(engine, documents) {
