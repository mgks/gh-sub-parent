...
function resolveSchema(schema, spec, _depth = 0) {
    if (!schema)
        return {};
    if (schema.$ref)
...
function typeLabel(schema, spec) {
    ...
    if (resolved.$ref)
        return resolved.$ref.split('/').pop() || 'object';
    if (resolved.type === 'array')
        return `array[${typeLabel(resolved.items, spec)}]`;
    if (resolved.oneOf)
        return resolved.oneOf.map(s => typeLabel(s, spec)).join(' | ');
    if (resolved.anyOf)
        return resolved.anyOf.map(s => typeLabel(s, spec)).join(' | ');
    if (resolved.enum)
...
function renderSchemaTable(schema, spec, options) {
    if (!schema)
        return '';
    const resolved = resolveSchema(schema, spec);
...
function renderOperation(method, path_, op, spec, options) {
...
function renderSpec(specPath, rootDir, options) {
 ...
 * ```
 */
export function markdownSetup(md, options) {
    const srcDir = options?.config?.src
