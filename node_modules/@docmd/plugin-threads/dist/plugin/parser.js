 ...
 */
export function findThreadsBlockBounds(lines) {
    let start = -1;
 ...
 */
export function parseChildContainers(lines) {
    const children = [];
    let i = 0;
...
export function parseThreadInfo(info) {
    const resolvedMatch = info.match(/^thread\s+(\S+)\s+resolved\s+"([^"]+)"\s+"([^"]+)"$/);
    if (resolvedMatch) {
        return {
            id: resolvedMatch[1],
            resolved: true,
...
export function parseCommentInfo(info) {
    ...
    const fullMatch = info.match(/^comment\s+(\S+)\s+"([^"]+)"\s+"([^"]+)"(?:\s+reply-to\s+(\S+))?(?:\s+edited\s+"([^"]+)")?$/);
    if (fullMatch) {
        return {
            id: fullMatch[1],
            author: fullMatch[2],
    ...
    const editedMatch = info.match(/^comment\s+"([^"]+)"\s+"([^"]+)"\s+edited\s+"([^"]+)"$/);
    if (editedMatch) {
        return {
            id: null,
            parent_id: null,
 ...
 */
export function parseReactions(lines) {
    const reactions = [];
 ...
 */
export function parseCommentContent(contentLines) {
 ...
 */
export function parseThreadsFromContent(markdownContent) {
 ...
 */
export function serializeThreadsBlock(threads) {
    if (threads.length === 0)
        return '';
    const lines = ['::: threads'];
 ...
 */
export function replaceThreadsBlock(markdownContent, threads) {
    const lines = markdownContent.split('\n');
    const bounds = findThreadsBlockBounds(lines);
