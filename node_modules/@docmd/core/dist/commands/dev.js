...
import { formatPathForDisplay, getNetworkIp, serveStatic, findAvailablePort, openBrowser, } from '../utils/dev-utils.js';
const __dirname = path.dirname(new URL(import.meta.url).pathname);
const require = createRequire(import.meta.url);
// Main Dev Function
export async function startDevServer(configPathOption, opts = {}) {
    const options = {
    ...
    async function reloadConfigAndRebuild(changedFilePath) {
        if (configLock)
            return;
        configLock = true;
    ...
    function setupContentWatchers() {
        ...
        if (nativeFs.existsSync(paths.userAssetsDir))
            contentPaths.push(paths.userAssetsDir);
        if (process.env.DOCMD_DEV === 'true') {
            const DOCMD_ROOT = path.resolve(__dirname, '..');
            contentPaths.push(path.join(DOCMD_ROOT, 'templates'), path.join(DOCMD_ROOT, 'assets'), path.join(DOCMD_ROOT, 'engine'), path.join(DOCMD_ROOT, 'plugins'), path.join(DOCMD_ROOT, 'utils'));
        }
        ...
        for (const watchPath of contentPaths) {
            if (!nativeFs.existsSync(watchPath))
                continue;
            const watcher = nativeFs.watch(watchPath, { recursive: true }, (event, filename) => {
                ...
                if (Date.now() - lastContentRebuildAt < CONTENT_QUIET_MS)
                    return;
                if (Date.now() - lastConfigRebuildAt < CONFIG_QUIET_MS)
                    return;
                ...
                if (!signatureTracker.hasChanged(filePath))
                    return;
                ...
                const isConfigOrNav = filename.includes('navigation.json') ||
    ...
    const setupConfigWatcher = () => {
        ...
        const configWatcher = nativeFs.watch(watchDir, (event, filename) => {
            ...
            if (!signatureTracker.hasChanged(configWatchPath))
                return;
    ...
    function tryStartServer(port) {
        server.listen(port, BIND_HOST)
            ...
            wss.on('error', (e) => TUI.error('WebSocket Error', e.message));
            ...
            if (devDetails.engine)
                TUI.item('Engine', devDetails.engine === 'rust' ? 'rust (preview)' : devDetails.engine, TUI.dim, TUI.green);
            if (devDetails.versions)
                TUI.item('Versions', `${devDetails.versions.count} (${devDetails.versions.labels})`, TUI.dim, TUI.green);
            if (devDetails.locales)
                TUI.item('Locales', `${devDetails.locales.count} (${devDetails.locales.labels})`, TUI.dim, TUI.green);
            TUI.item('', '', TUI.dim, TUI.green);
    ...
    async function gracefulShutdown() {
        ...
        try {
            const closures = [];
            watchers.forEach(w => closures.push(new Promise(resolve => { w.close(); resolve(); })));
            if (wss)
                closures.push(new Promise(resolve => wss.close(resolve)));
            if (server)
                closures.push(new Promise(resolve => server.close(resolve)));
            if (workerPool)
                closures.push(workerPool.terminateAll());
