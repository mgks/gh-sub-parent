...
const pkg = JSON.parse(nativeFs.readFileSync(_pkgUrl, 'utf8'));
export async function buildSite(configPath, opts = {}) {
    ...
    try {
        ...
        if (loadErrors.length > 0) {
            ...
            throw new Error(`Build failed: ${loadErrors.length} plugin(s) could not be loaded:\n` +
                lines.map((l) => `  - ${l}`).join('\n'));
        }
                ...
                for (const getAssetsFn of hooks.assets) {
                    ...
                    if (Array.isArray(assets)) {
                        for (const asset of assets) {
                            ...
                            const src = asset.src ?? asset.path;
                            const dest = asset.dest ?? asset.url;
                            if (src && dest) {
        ...
        const indexingHooks = hooks.onPostBuild.filter((fn) => INDEXING_PLUGINS.has(fn._pluginName));
        const publishingHooks = hooks.onPostBuild.filter((fn) => !INDEXING_PLUGINS.has(fn._pluginName));
        const hasIndexingWork = !options.targetFiles && ((hooks.onBeforeBuild?.length ?? 0) > 0 || indexingHooks.length > 0);
        if (hasIndexingWork && !options.quiet)
        ...
        if (config.i18n && config.i18n.locales) {
            ...
            const localeIds = new Set(config.i18n.locales.map((l) => l.id));
            const manifest = {};
        ...
        if (config.redirects && Object.keys(config.redirects).length > 0) {
            for (const [from, to] of Object.entries(config.redirects)) {
                ...
                if (!cleanFrom.endsWith('.html'))
                    cleanFrom = path.join(cleanFrom, 'index.html');
        ...
        if (!options.targetFiles) {
            ...
            if (publishingHooks.length > 0) {
                ...
                for (const fn of publishingHooks) {
                    const pluginName = String(fn._pluginName || 'plugin');
                    const entries = [];
                    const pluginCtx = {
                        ...postBuildCtx,
                        log: (msg, status = 'DONE') => {
                    ...
                    await fn(pluginCtx);
                    if (!options.quiet)
                        TUI.pluginTree(pluginName, entries, TUI.blue);
                }
                if (!options.quiet)
                    TUI.footer(TUI.blue);
            }
