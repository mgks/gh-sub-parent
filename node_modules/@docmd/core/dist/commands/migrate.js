...
export async function migrateProject(options) {
    ...
    const moveFilesToBackup = async (backupDir) => {
        ...
        for (const file of files) {
            ...
            if (file === 'node_modules' || file === '.git' || file === backupName || file === 'docmd.config.js')
                continue;
            if (file === 'package.json' || file === 'package-lock.json' || file === 'yarn.lock' || file === 'pnpm-lock.yaml' || file === 'bun.lockb' || file === 'bun.lock')
                continue;
     ...
     */
    const planBackupMoves = async (backupName) => {
        const files = await nativeFs.promises.readdir(CWD);
     ...
     */
    const printAndExitDryRun = async (sourceName, backupName, docmdConfig) => {
        ...
        if (moves.length > 0 && moves.length <= 12) {
            ...
            for (const f of moves.slice(0, 10))
                TUI.item(' ', f);
     ...
     */
    const translateDocusaurusFrontmatter = async (docsDir) => {
        const targets = [];
    ...
    if (options.docusaurus) {
        ...
        else if (nativeFs.existsSync(tsConfigPath))
        ...
        if (staticDirMatch)
            out = staticDirMatch[1].replace(/^\.\//, '').replace(/\/$/, '') || 'static';
