...
const _pkg = JSON.parse(nativeFs.readFileSync(_pkgUrl, 'utf8'));
const CORE_VERSION = _pkg.version;
/* ── Constants ────────────────────────────────────────────────── */
...
export async function renderPages({ config, srcDir, fallbackSrcDir, outputDir, hooks, buildHash, options, outputPrefix = '', coreVersion, onProgress, targetFiles }) {
    ...
    const pluginTranslations = hooks.translations
        ? await hooks.translations.reduce(async (accP, fn) => ({ ...(await accP), ...(await fn(localeId)) }), {})
        : {};
    ...
    const mdProcessor = parser.createMarkdownProcessor(configWithStrings, (md) => hooks.markdownSetup.forEach((hook) => hook(md)));
    ...
    if (themeName && themeName !== 'default' && themeName !== 'none' && !themeIsTemplate) {
        assetTags.head.push({
            priority: 5,
            gen: (rel) => generateAssetTag(`${rel}assets/css/docmd-theme-${themeName}.css?v=${buildHash}`, 'css'),
        });
    }
    // Lightbox (priority 20 — always last, no theme/template should override it)
    assetTags.body.push({
        priority: 20,
        gen: (rel) => generateAssetTag(`${rel}assets/js/docmd-image-lightbox.js?v=${buildHash}`, 'js'),
    });
    ...
    if (hooks.templateAssets && Array.isArray(hooks.templateAssets)) {
        for (const asset of hooks.templateAssets) {
            if (!asset || !asset.path || (asset.type !== 'css' && asset.type !== 'js'))
                continue;
            const baseName = path.basename(asset.path);
            const position = (asset.position || (asset.type === 'css' ? 'head' : 'body'));
            ...
            bucket.push({
                priority,
                gen: (rel) => generateAssetTag(`${rel}assets/template/${baseName}?v=${buildHash}`, asset.type),
            });
        }
    }
    // Plugin Assets
            ...
            if (Array.isArray(assets)) {
                for (const asset of assets) {
                    ...
                    if (tagGen) {
                        ...
                        bucket.push({ priority, gen: tagGen, condition: asset.condition });
                    }
                }
            }
        }
    ...
    const navDesignatedIndexFiles = new Set();
    const extractNavIndexes = (items) => {
    ...
    if (config.navigation)
        extractNavIndexes(config.navigation);
    ...
    const localeIds = new Set((config._allLocales || []).map((l) => l.id));
    ...
    if (fallbackSrcDir && nativeFs.existsSync(srcDir)) {
        ...
        for (const filePath of localeFiles) {
            ...
            if (fallbackRelPaths.has(relativePath))
                continue;
    ...
    for (let batchStart = 0; batchStart < filteredManifest.length; batchStart += BATCH_SIZE) {
        ...
        const fileContents = await Promise.all(batch.map(entry => nativeFs.promises.readFile(entry.targetFilePath, 'utf8')));
        ...
        for (let j = 0; j < batch.length; j++) {
            ...
            if (isFallback) {
                const defaultLabel = config._allLocales?.find((l) => l.id === config._defaultLocale)?.label || config._defaultLocale;
            ...
            if (ext === '.ejs') {
                try {
                    ...
                    TUI.warn(`Skipping EJS render error in ${relativePath}: ${e.message}`);
                    continue;
                }
            }
            ...
            const withoutExt = slugifyOutputPath(relativePath.replace(/\.(md|markdown|ejs)$/, '').replace(/\\/g, '/'));
            const slugifiedDir = slugifyOutputPath(path.dirname(relativePath).replace(/\\/g, '/'));
            ...
            if (relativePathToRootForMarkdown === '')
                relativePathToRootForMarkdown = './';
            else
                relativePathToRootForMarkdown += '/';
            relativePathToRootForMarkdown = relativePathToRootForMarkdown.replace(/\\/g, '/');
            const processed = await parser.processContentAsync(rawContent, mdProcessor, config, {
                ...
                allLocales: (config._allLocales || []).map((l) => l.id),
    ...
    for (let i = 0; i < pages.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (page) => {
            ...
            if (siteRootAbs !== '/' && !siteRootAbs.endsWith('/'))
                siteRootAbs += '/';
            ...
            const buildRelativeUrl = (href) => buildContextualUrl(href, urlContext);
            ...
            const buildAbsoluteUrlContextual = (base, localePrefix, versionPrefix, pagePath) => buildAbsoluteContextualUrl(base, localePrefix, versionPrefix, pagePath, urlContext);
            // Fix Neighbor Links
            ...
            const matchesCondition = (condition) => {
                ...
                if (condition.pageHtmlMatches != null) {
                    const needles = Array.isArray(condition.pageHtmlMatches)
                        ...
                        : [condition.pageHtmlMatches];
                    const anyHit = needles.some((n) => typeof n === 'string' && n.length > 0 && html.includes(n));
                    if (!anyHit)
                        return false;
                }
                if (condition.frontmatterHas != null) {
                    if (typeof condition.frontmatterHas !== 'string')
                        return false;
                    if (!Object.prototype.hasOwnProperty.call(fm, condition.frontmatterHas))
                        return false;
                }
                return true;
            };
            const renderAssetList = (entries) => entries
            ...
            const headInjections = await Promise.all(hooks.injectHead.map((fn) => fn(config, pageContext, relativePathToRoot)));
            const bodyInjections = await Promise.all(hooks.injectBody.map((fn) => fn(config, pageContext)));
    ...
    for (let i = 0; i < writeQueue.length; i += WRITE_BATCH_SIZE) {
        ...
        await Promise.all(writeBatch.map(async ({ finalPath, html }) => {
