 ...
 */
export async function filterGhostVersions(config, CWD, isDev) {
    ...
    for (const v of config.versions.all) {
        ...
        if (await fs.exists(vSrcDir)) {
            ...
            if (isCurrent) {
                throw new Error(`Current version directory missing: ${vSrcDir} (version: ${v.id}). ` +
            ...
            if (!isDev)
                TUI.warn(`Skipping missing version: ${v.id} (${v.dir})`);
        }
    }
 ...
 */
export function filterNavForVersion(items, vSrcDir, fallbackSrcDir) {
            ...
            if (relativeFilePath.endsWith('/') || relativeFilePath === '') {
                ...
                const found = indexCandidates.find(c => {
                    ...
                    if (nodeFs.existsSync(abs))
                        return true;
                    if (fallbackSrcDir && nodeFs.existsSync(path.join(fallbackSrcDir, c)))
                        return true;
                    return false;
                });
            ...
            try {
                if (!nodeFs.existsSync(absoluteFilePath)) {
                    ...
                    if (fallbackSrcDir) {
                        ...
                        try {
                            if (!nodeFs.existsSync(fallbackFilePath))
                                return acc;
                        }
                        catch {
                            return acc;
 ...
 */
export function resolveVersionNav(v, vSrcDir, configNavigation) {
 ...
 */
export async function buildVersions({ config, outputDir, hooks, buildHash, options, CWD, pathPrefix = '', onProgress, targetFiles, coreVersion }) {
    const allPages = [];
    // Pre-scan: determine which versions have i18n support for the current locale
    const versionI18nMap = {};
    for (const v of config.versions.all) {
    ...
    for (const v of config.versions.all) {
        ...
        if (!await fs.exists(vSrcDir)) {
            if (!options.isDev)
                TUI.warn(`Version directory missing: ${v.dir}. Skipping ${v.id}...`);
            continue;
        }
