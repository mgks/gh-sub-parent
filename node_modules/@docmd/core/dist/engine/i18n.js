 ...
 */
export async function preCountPages(config, CWD, targetFiles) {
    ...
    for (const locale of locales) {
        ...
        for (const v of versions) {
            ...
            if (!nativeFs.existsSync(localeSrcDir)) {
                if (!isDefault && localeId)
                    continue; // non-default locale: skip
            ...
            if (!nativeFs.existsSync(scanDir))
                continue;
 ...
 */
export function createLocaleConfig(config, locale) {
    if (!locale)
        return config;
 ...
 */
export function resolveLocaleSrcDir(baseSrcDir, config) {
    if (!config._activeLocale)
        return baseSrcDir;
    if (config.i18n?.stringMode)
        return baseSrcDir;
 ...
 */
export function resolveFallbackSrcDir(baseSrcDir, config) {
    if (!config._activeLocale || !config._defaultLocale)
        return null;
    if (config.i18n?.stringMode)
        return null;
    if (config._activeLocale.id === config._defaultLocale)
        return null;
 ...
 */
export function getLocales(config) {
 ...
 */
export async function buildLocales({ config, rootOutputDir, hooks, buildHash, options, CWD, onProgress, targetFiles, coreVersion }) {
    ...
    if (config.i18n && config.i18n.locales) {
        ...
        const availableLocaleIds = new Set();
        for (const loc of config.i18n.locales) {
            ...
            if (config.versions?.all?.length > 0) {
                const currentVersion = config.versions.all.find((v) => v.id === config.versions.current);
                if (currentVersion) {
    ...
    for (const locale of locales) {
        ...
        if (isStringMode && localeId && !isDefault) {
            ...
            for (const page of defaultPassPages) {
                ...
                if (!nativeFs.existsSync(defaultOutputPath))
                    continue;
        ...
        if (localeConfig.versions?.all?.length > 0) {
            ...
            if (isStringMode && isDefault)
                defaultPassPages = pages;
        }
        else {
            ...
            allGeneratedPages.push(...pages);
            if (isStringMode && (isDefault || !localeId))
                defaultPassPages = pages;
        }
    }
 ...
 */
export async function generateLocaleRedirect(config, _rootOutputDir) {
 ...
 */
export function generateHreflangTags(config, pageOutputPath) {
    if (!config._allLocales)
        return '';
    ...
    return config._allLocales.map((loc) => {
 ...
 */
export async function loadStringModeTranslations(CWD, assetsDir, localeId) {
 ...
 */
export function applyStringModeReplacements(html, strings, localeId, localeDir) {
    if (!strings || Object.keys(strings).length === 0)
        return html;
    // 1. data-i18n="key" → replace textContent
    //    Match: <tag ... data-i18n="key" ...>content</tag>
    html = html.replace(/(<[^>]+\sdata-i18n="([^"]+)"[^>]*>)([\s\S]*?)(<\/[a-zA-Z][a-zA-Z0-9]*>)/g, (match, openTag, key, _content, closeTag) => {
    ...
    html = html.replace(/(<[^>]+\sdata-i18n-html="([^"]+)"[^>]*>)([\s\S]*?)(<\/[a-zA-Z][a-zA-Z0-9]*>)/g, (match, openTag, key, _content, closeTag) => {
        if (strings[key] !== undefined) {
            return openTag + strings[key] + closeTag;
        }
        return match;
    ...
    html = html.replace(/(<[^>]+)\sdata-i18n-(?!html)([a-zA-Z][\w-]*)="([^"]+)"([^>]*>)/g, (match, before, targetAttr, key, after) => {
        if (strings[key] !== undefined) {
    ...
    html = html.replace(/window\.DOCMD_LOCALE\s*=\s*"[^"]*"/, `window.DOCMD_LOCALE = "${localeId}"`);
    return html;
}
