...
async function init() {
    ...
    mdProcessor = createMarkdownProcessor(configWithStrings, (md) => {
        hooks.markdownSetup.forEach((hook) => hook(md));
    });
 ...
 * any real message.
 */
async function verifyDeterminismAtBoot() {
    const result = await processContentAsync(DETERMINISM_SNAPSHOT_INPUT, mdProcessor, config, { filePath: '<determinism-snapshot>' }, hooks);
    if (result.htmlContent !== DETERMINISM_SNAPSHOT_OUTPUT) {
        throw new Error('[docmd] Parser determinism regression detected at worker boot.\n' +
