 ...
 * License: MIT
 */`;
export async function findFilesRecursive(dir, extensions) {
    let files = [];
    ...
    for (const item of items) {
        ...
        if (item.name === 'node_modules' || item.name.startsWith('.') || item.name === 'site')
            continue;
...
export async function prepareAssets(config, outputDir, options = {}) {
    ...
    if (await fs.exists(userAssets))
        await fs.copy(userAssets, path.join(outputDir, 'assets'));
    // 3.5. User Assets (Docs Dir)
    if (config.src) {
        const srcAssets = path.resolve(CWD, config.src, 'assets');
        if (await fs.exists(srcAssets))
            await fs.copy(srcAssets, path.join(outputDir, 'assets'));
    }
...
// ---------------------------------------------------------------------------
export async function prepareTemplateAssets(config, outputDir) {
    ...
    if (!hooks || !Array.isArray(hooks.templateAssets) || hooks.templateAssets.length === 0)
        return;
    const templateDir = path.join(outputDir, 'assets', 'template');
    await fs.ensureDir(templateDir);
    for (const asset of hooks.templateAssets) {
        if (!asset || !asset.path || !asset.type)
            continue;
...
export function generateAssetTag(pathOrUrl, type, attributes = {}) {
    const attrs = Object.entries(attributes).map(([k, v]) => v === true ? k : `${k}="${v}"`).join(' ');
    if (type === 'css')
        return `<link rel="stylesheet" href="${pathOrUrl}" ${attrs}>`;
    if (type === 'js')
        return `<script src="${pathOrUrl}" ${attrs}></script>`;
    return '';
}
