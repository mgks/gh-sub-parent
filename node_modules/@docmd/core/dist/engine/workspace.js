 ...
 */
export function isWorkspace(rawConfig) {
    if (!rawConfig)
        return false;
    // New 'workspace' schema
    if (rawConfig.workspace && Array.isArray(rawConfig.workspace.projects)) {
        return rawConfig.workspace.projects.every((p) => typeof p.prefix === 'string' && typeof p.src === 'string');
    }
    // Legacy 'projects' schema (for backward compatibility)
    if (Array.isArray(rawConfig.projects) && rawConfig.projects.length > 0) {
        return rawConfig.projects.every((p) => typeof p.prefix === 'string' && typeof p.src === 'string');
    }
    return false;
}
/**
 ...
 */
export async function detectWorkspace(configPathOption) {
...
function validateProjects(projects) {
    ...
    if (!hasRoot) {
        throw new Error('Workspace configuration must include a root project with prefix "/".\n' +
 ...
 */
export async function buildWorkspace(workspaceConfig, opts = {}) {
    const CWD = process.cwd();
...
async function buildWorkspaceProject(project, workspaceConfig, opts = {}) {
    const CWD = process.cwd();
    const rootOutDir = path.resolve(CWD, workspaceConfig.out || 'site');
    const workspace = workspaceConfig.workspace;
    const prefix = project.prefix === '/' ? '/' : project.prefix.replace(/\/$/, '');
 ...
 */
export async function devWorkspace(workspaceConfig, opts = {}) {
    try {
    ...
    for (const project of workspace.projects) {
        const projectSrcDir = path.resolve(CWD, project.src);
        if (!nativeFs.existsSync(projectSrcDir))
            continue;
        nativeFs.watch(projectSrcDir, { recursive: true }, (event, filename) => {
            ...
            if (filename.includes('.git') || filename.includes('node_modules') ||
                ...
                filename.includes('docmd.config-') || filename.endsWith('.js.bak'))
                return;
    ...
    if (workspaceConfig._resolvedPath && nativeFs.existsSync(workspaceConfig._resolvedPath)) {
        ...
        nativeFs.watch(rootConfigDir, (event, filename) => {
            if (!filename)
                return;
            if (filename !== rootConfigBaseName)
                return;
            // Post-rebuild quiet period
            if (Date.now() - lastConfigRebuildAt < CONFIG_QUIET_MS)
                return;
            // Content-signature gate: ignore phantom events that don't actually
            // mutate the file.
            if (!signatureTracker.hasChanged(rootConfigPath))
                return;
            if (rootConfigDebounce)
                clearTimeout(rootConfigDebounce);
