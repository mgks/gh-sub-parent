 ...
 */
const KNOWN_CSS_THEMES = new Set([
    'default',
    'sky',
    'ruby',
    'retro',
...
export function normalizeConfig(userConfig) {
    ...
    if (_userSetBaseExplicitly && config.base !== '/') {
        ...
        b = b.replace(/([^:])\/{2,}/g, '$1/');
        if (b !== config.base)
            config._baseNormalised = config.base;
        config.base = b;
    }
    ...
    if (config.pageNavigation === undefined)
        config.pageNavigation = true;
    if (config.copyCode === undefined)
        config.copyCode = true;
    if (config.autoTitleFromH1 === undefined)
        config.autoTitleFromH1 = true;
    ...
    if (config.menubar) {
        ...
        if (config.menubar.left)
            normalizeItems(config.menubar.left);
        if (config.menubar.right)
            normalizeItems(config.menubar.right);
    }
    // --- 5. Plugins ---
    ...
    if (config.i18n && config.i18n.locales && Array.isArray(config.i18n.locales) && config.i18n.locales.length > 0) {
        config.i18n = {
            ...
            locales: config.i18n.locales.map((loc) => ({
                id: loc.id,
