...
function hasMarkdownFiles(dir, maxDepth = 2, currentDepth = 0) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.name === 'node_modules' || entry.name.startsWith('.') || entry.name === 'site' || entry.name === 'dist' || entry.name === 'out')
                continue;
            if (entry.isFile() && (entry.name.endsWith('.md') || entry.name.endsWith('.markdown') || entry.name.endsWith('.ejs')))
                return true;
            if (entry.isDirectory()) {
                if (hasMarkdownFiles(path.join(dir, entry.name), maxDepth, currentDepth + 1))
                    return true;
            }
        }
    }
...
function detectHeuristics(cwd, srcDir) {
    ...
    const isVersion = (name) => /^v\d/i.test(name);
    const isLocale = (name) => /^[a-z]{2}(-[A-Z]{2})?$/i.test(name);
...
async function buildZeroConfig(cwd, isDev = false, quiet = false, options = {}) {
    if (isDev && !quiet) {
    ...
    if (!srcDir) {
        ...
        const err = new Error('No candidate documentation directory or files found.');
        err.silent = true;
        throw err;
    }
    const absSrcDir = path.join(cwd, srcDir);
...
export async function loadConfig(configPath, options = {}) {
    ...
    try {
        ...
        try {
            ...
            const isLegacy = !rawConfig.layout && (rawConfig.search !== undefined ||
            ...
            // Users can opt out of auto-nav entirely by setting `autoNav: false`.
                ...
                if (normalized.i18n?.default) {
                    ...
                    hasNavInSubdirs = (normalized.i18n.locales || []).some((l) => fs.existsSync(path.join(path.resolve(cwd, normalized.srcDir), l.id, 'navigation.json')));
                }
                // Check if any version dir has navigation.json
                if (!hasNavInSubdirs && normalized.versions?.all?.length > 0) {
                    hasNavInSubdirs = normalized.versions.all.some((v) => {
                        const vDir = path.resolve(cwd, v.dir);
                        if (fs.existsSync(path.join(vDir, 'navigation.json')))
                            return true;
                        if (normalized.i18n?.default) {
                ...
                if (!hasNavInSubdirs) {
                    if (!options.quiet && !global.__DOCMD_ZERO_NAV_LOGGED) {
                        ...
                        if (options.isDev)
                            global.__DOCMD_ZERO_NAV_LOGGED = true;
                    }
