 ...
 */
function escapeLineTerminators(json) {
    return json
        .replace(/\u2028/g, '\\u2028')
        .replace(/\u2029/g, '\\u2029');
}
 ...
 */
export function jsonInject(value) {
 ...
 */
export function sanitizeHeadInjection(html) {
    ...
    out = out.replace(/(<[a-z][a-z0-9]*\b[^>]*?\s(?:href|src))\s*=\s*"javascript:[^"]*"/gi, '$1="#"');
    out = out.replace(/(<[a-z][a-z0-9]*\b[^>]*?\s(?:href|src))\s*=\s*'javascript:[^']*'/gi, "$1='#'");
    out = out.replace(/(<[a-z][a-z0-9]*\b[^>]*?\s(?:href|src))\s*=\s*"vbscript:[^"]*"/gi, '$1="#"');
    out = out.replace(/(<[a-z][a-z0-9]*\b[^>]*?\s(?:href|src))\s*=\s*'vbscript:[^']*'/gi, "$1='#'");
    return out;
}
