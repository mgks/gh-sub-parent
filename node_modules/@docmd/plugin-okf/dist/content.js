...
export function resolveType(fm, pathname, defaultType, typeField = 'type') {
    const fmType = (fm?.okf?.type)
        ...
        || null;
    if (fmType)
        return { type: String(fmType), fallback: false };
    for (const [re, t] of PATH_TYPE_MAP)
        if (re.test(pathname))
            return { type: t, fallback: false };
 ...
 * are skipped.
 */
export function extractInternalLinks(md, ownSlug, known) {
    const out = [];
    if (!md)
        return out;
    const re = /\[[^\]]*\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g;
