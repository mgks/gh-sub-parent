 ...
 */
export function serializeConceptFrontmatter(fm) {
    const lines = [];
    for (const k of Object.keys(fm)) {
        ...
        if (Array.isArray(v)) {
            ...
            lines.push(`${k}:`);
            for (const it of v)
                lines.push(`  - ${typeof it === 'string' ? yamlQuote(it) : String(it)}`);
        }
        else if (v && typeof v === 'object') {
            lines.push(`${k}:`);
            for (const kk of Object.keys(v))
                lines.push(`  ${kk}: ${typeof v[kk] === 'string' ? yamlQuote(v[kk]) : String(v[kk])}`);
        }
