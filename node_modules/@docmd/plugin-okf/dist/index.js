...
export async function onPostBuild({ config, pages, outputDir, log }) {
    ...
    if (_opts === false)
        return; // `"okf": false`
    if (_opts && _opts.enabled === false)
        return; // `"okf": { "enabled": false }`
    if (Array.isArray(_opts?.capabilities)
        && !_opts.capabilities.includes('post-build'))
    ...
    const graphEnabled = opts.graph === true ||
    ...
    if (log)
        log(`Generating OKF bundle: ${bundleName}`);
    const localeIds = (config.i18n?.locales || []).map((l) => l.id);
    const versionIds = (config.versions?.all || []).map((v) => v.id);
    ...
    const filtered = pages.filter((p) => {
        ...
        if (fm.noindex || fm.okf === false)
            return false;
        const pathname = outputPathToPathname(p.outputPath);
        if (matchesPattern(pathname, excludePatterns))
            return false;
        if (matchesPattern(p.outputPath || '', excludePatterns))
            return false;
    ...
    for (const p of filtered) {
        ...
        if (localeStrategy === 'folders' && locale && locale !== defaultLocale && localeIds.length > 1)
            subParts.push(locale);
        if (versionStrategy === 'folders' && version && versionIds.length > 1)
            subParts.push(version);
        ...
        const conceptFm = { [typeField]: type };
        if (fm.title)
            conceptFm.title = fm.title;
        if (fm.description)
            conceptFm.description = fm.description;
        conceptFm.source = fullUrl;
        conceptFm.path = pathname;
        if (locale)
            conceptFm.locale = locale;
        if (version)
            conceptFm.version = version;
        if (Array.isArray(fm.tags) && fm.tags.length)
            conceptFm.tags = fm.tags;
        ...
        if (includeFullMarkdown) {
            if (p.sourcePath) {
                try {
                    body = await fs.readFile(p.sourcePath, 'utf8');
                }
                catch {
                    body = '';
                }
        ...
        if (!body && typeof p.rawMarkdown === 'string')
            body = p.rawMarkdown;
    ...
    for (const c of concepts)
        if (!inbound.has(c.id))
            warnings.push(`[WARN] orphan-concept ${c.id}  (no inbound links)`);
    for (const l of linkList)
        if (!allSlugs.has(l.target))
            warnings.push(`[WARN] broken-link ${l.source} -> ${l.target}`);
    const byType = {};
    for (const c of concepts)
    ...
    const idxLines = [`# ${manifest.bundle.title} — Knowledge Catalog`, '', `> Generated by ${manifest.bundle.generated_by} on ${manifest.bundle.generated_at}`, ''];
    if (manifest.bundle.description)
        idxLines.push(manifest.bundle.description, '');
    const catalogLinks = ['- [Manifest](okf.yaml)', '- [Bundle summary (JSON)](_meta/bundle.json)', '- [Lint report](_meta/lint-report.txt)'];
    if (graphEnabled)
        catalogLinks.splice(2, 0, '- [Graph viewer](graph/)');
    ...
    if (untyped.length && Object.keys(byType).length > 1) {
        idxLines.push(`## Untyped (using fallback \`${defaultType}\`)`, '', 'These pages did not declare an explicit OKF type. Consider adding `type:` to their frontmatter:', '');
        for (const c of untyped)
            idxLines.push(`- [${c.title}](${c.file})`);
        idxLines.push('');
    }
    ...
    if (missingTypeCount > 0 && warnOnMissingType && log) {
        log(`OKF: ${missingTypeCount} page${missingTypeCount === 1 ? '' : 's'} missing explicit type (using fallback '${defaultType}') — add \`type:\` to frontmatter or set \`warnOnMissingType: false\` to silence`, 'SKIP');
    }
    if (log)
        log(`OKF bundle written to /${outputRel}/ (${concepts.length} concepts, ${warnings.length} warnings)`);
}
