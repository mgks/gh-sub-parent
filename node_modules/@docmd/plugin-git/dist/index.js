 ...
 */
async function getGitLogViaEngine(filePaths, maxCommits) {
...
function saveDiskCache() {
    if (!_cacheDirty || !_diskCachePath || !_diskCache)
        return;
    try {
...
async function getGitFileInfo(filePath, maxCommits = 6) {
 ...
 */
function formatLastUpdated(timestamp, locale = 'en') {
    const now = Date.now();
 ...
 */
export function onBeforeParse(_ctx) {
    // Logic removed (moved to onConfigResolved for better performance)
}
export async function onBeforeBuild(ctx) {
    ...
    const buildId = options?._buildId || '';
    ...
    if (showTui)
        tui.step(`Syncing git metadata`, 'WAIT');
    ...
    for (const page of pages) {
        ...
        if (sourcePath && page.frontmatter && !gitCache.has(sourcePath)) {
            ...
            if (fileStat && _diskCache && _diskCache[sourcePath] && _diskCache[sourcePath].mtimeMs === fileStat.mtimeMs) {
                ...
                if (page.frontmatter)
                    page.frontmatter._git = _diskCache[sourcePath].info;
            }
            else {
                pathsToFetch.push(sourcePath);
                if (!pagesByPath.has(sourcePath))
                    pagesByPath.set(sourcePath, []);
    ...
    if (engineResult && engineResult.size > 0) {
        ...
        for (const [filePath, commits] of engineResult) {
            if (commits && commits.length > 0) {
                ...
                for (const page of pagesForPath) {
                    if (page.frontmatter)
                        page.frontmatter._git = info;
                }
            }
        }
        if (showTui)
            tui.step(`Syncing git metadata`, 'DONE');
    }
    else {
        ...
        for (let i = 0; i < total; i += CONCURRENCY) {
            ...
            if (showTui)
                tui.step(`Syncing git metadata  ${processed}/${total}`, 'WAIT');
        }
        if (showTui)
            tui.step(`Syncing git metadata`, 'DONE');
    }
 ...
 */
export function getAssets(_options) {
