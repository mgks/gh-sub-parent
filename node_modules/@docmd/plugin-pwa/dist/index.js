...
export async function onPostBuild({ config, outputDir, log }) {
    ...
    if (pwaConfig.enabled === false)
        return; // Enabled by default if loaded
    if (log)
        log('Generating PWA assets');
    ...
    if (!icons) {
        ...
        if (!logoUrl && config.logo) {
            if (typeof config.logo === 'string')
                logoUrl = config.logo;
            else if (config.logo.light)
                logoUrl = config.logo.light;
        }
  ...
  `;
    await fs.writeFile(path.join(outputDir, 'service-worker.js'), swContent.trim());
}
