 ...
 */
export function generateMetaTags(config, pageData, _relativePathToRoot) {
    let html = '';
 ...
 */
export async function onPostBuild({ config, outputDir, log }) {
    ...
    if (existingRobotsPath) {
        if (existingRobotsPath !== robotsPath) {
            ...
            if (log)
                log('Copied robots.txt from assets to site root');
        }
        else {
            if (log)
                log('robots.txt already exists in site root, preserving');
        }
        return;
    }
    ...
    if (!nativeFs.existsSync(nojekyllPath)) {
        ...
        if (log)
            log('Generated .nojekyll (disables Jekyll on GitHub Pages)');
    }
}
