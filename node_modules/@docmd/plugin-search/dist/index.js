...
import { outputPathToSlug, installPackages, manualResolvePackageEntry } from '@docmd/api';
 ...
 */
const _searchState = { lastPeersHash: null, lastConfig: null };
/**
 ...
 */
function buildSearchConfig(config) {
...
function resolveDocmdSearch() {
    ...
    try {
        ...
        const mainEntry = (exp && (exp.import || exp.default || exp.require)) ||
            pkg.main ||
            'dist/index.js';
 ...
 */
async function getLatestDocmdSearchVersion() {
    try {
 ...
 */
async function autoInstallDocmdSearch(tui, quiet) {
 ...
 */
async function installPeerDeps(tui, quiet) {
    const cwd = process.cwd();
    if (!quiet && tui) {
    ...
    if (success) {
        if (!quiet && tui)
            tui.step('Peer dependencies installed', 'DONE');
        return true;
    }
    else {
 ...
 */
function findMissingPeerDep(resolvePaths) {
 ...
 */
async function ensureDocmdSearch(tui, quiet) {
    const resolvePaths = [process.cwd(), path.join(process.cwd(), 'node_modules')];
 ...
 */
function maybeReportPeerStatus(tui, sc, quiet) {
    if (quiet || !tui)
        return;
    if (!sc.semanticRequested)
        return; // user never asked for semantic — stay silent
    const h = peersHash(sc);
    if (h === _searchState.lastPeersHash)
    ...
    tui.warn('  Semantic search requires: ' + sc.missingPeers.join(', ') +
 ...
 */
async function stampSemanticFlag(outputDir) {
    ...
    await Promise.all(htmlFiles.map(async (file) => {
        try {
            ...
            if (!src.includes(MODAL_MARKER) || src.includes(FLAG))
                return;
            // Insert the flag right after the modal id attribute.
            const updated = src.replace(/(id="docmd-search-modal"[^>]*?)(\s)/, `$1 ${FLAG}$2`);
            if (updated !== src)
...
async function findHtmlFiles(dir) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            if (entry.name === '.docmd-search' || entry.name === 'node_modules')
                continue;
...
export async function onPostBuild({ config, pages, outputDir, tui, options, runWorkerTask }) {
    ...
    if (pluginOptions.semantic === true) {
        ...
        if (nativeFs.existsSync(clientDestPath)) {
            try {
                ...
                if (stripped !== src)
                    await fs.writeFile(clientDestPath, stripped, 'utf8');
            }
            catch { /* non-critical */ }
        }
            ...
            if (nativeFs.existsSync(manifestPath)) {
                try {
                    ...
                    if (manifest.status === 'complete' && manifest.batchCount > 0) {
                        if (showTui)
                            tui.step('Using pre-built semantic index', 'DONE');
        ...
        if (!ready) {
            ...
            // was missing).
            const why = reason === 'peers' ? 'semantic peer dependencies missing' :
            ...
            if (showTui)
                tui.warn(`  Falling back to keyword search (${why})`);
        }
            ...
            if (showTui)
                tui.step('Re-running semantic indexing in subprocess (first install)...', 'WAIT');
            try {
                const { execSync } = await import('node:child_process');
                const cwd = process.cwd();
                ...
                if (showTui)
                    tui.step('Semantic search index built', 'DONE');
            }
            catch (err) {
                if (showTui)
                    tui.warn('  Subprocess indexing failed — semantic search unavailable this build.');
            }
        }
        else {
            if (showTui)
                tui.step('Building semantic search index', 'WAIT');
            try {
                ...
                if (!docmdSearchPath) {
                    throw new Error('docmd-search not found. Install it with: npm install docmd-search');
                }
                // Import using the resolved path - this is safe and works with bundlers
                const docmdSearch = await import(docmdSearchPath);
                if (!docmdSearch?.indexDirectory) {
                    throw new Error('docmd-search found but indexDirectory not exported. ' +
                ...
                if (hasVersioning) {
                    ...
                    if (showTui)
                        tui.step('Building semantic search index (multi-version)', 'WAIT');
                    ...
                    for (const ver of versions) {
                        ...
                        try {
                            ...
                            if (showTui)
                                tui.warn(`  Skipping version ${ver.label}: ${verErr.message}`);
                            continue;
                        }
                        ...
                        if (nativeFs.existsSync(tmpBatchesDir)) {
                            ...
                            for (const batchFile of batchFiles) {
                                ...
                                if (ver.outputPrefix) {
                                    batchJson.chunks = batchJson.chunks.map((chunk) => ({
                                        ...chunk,
                                ...
                                await fs.writeFile(path.join(mergedBatchesDir, `${paddedId}.json`), JSON.stringify(batchJson));
                    ...
                    await fs.writeFile(path.join(semanticOutDir, 'versions.json'), JSON.stringify(versionMap));
                    // Clean up temp dirs
                    try {
                    ...
                    if (showTui)
                        tui.step('Building semantic search index (multi-version)', 'DONE');
                ...
                if (showTui)
                    tui.step('Building semantic search index', 'DONE');
                // Pages were rendered before deps installed; stamp the flag now.
                await stampSemanticFlag(outputDir);
    ...
    if (showTui && !semanticBuilt)
        tui.step('Generating search index', 'WAIT');
    ...
    if (runWorkerTask) {
        try {
            ...
            const serializablePages = pages
                .filter((p) => p.searchData)
                .map((p) => ({ outputPath: p.outputPath, searchData: p.searchData }));
            ...
            if (showTui && !semanticBuilt)
                tui.step('Generating search index', 'DONE');
            return;
        }
        catch {
 ...
 */
async function buildSearchIndexInline(config, pages, outputDir) {
 ...
 */
export function generateScripts(config, options) {
    const isEnabled = config.optionsMenu ? config.optionsMenu.components.search !== false : config.search !== false;
    if (!isEnabled)
        return {};
    ...
    const sc = config._searchConfig || buildSearchConfig(config);
