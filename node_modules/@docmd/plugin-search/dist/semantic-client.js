 ...
 */
export async function loadSemanticIndex(ctx) {
    ...
    await semanticClient.load(semanticIndexBase, (loaded, total) => {
    ...
    try {
        ...
        if (vRes.ok) {
            ...
            if (Array.isArray(vData) && vData.length > 0) {
                ...
                ctx.globalVersionColors.__semanticVersions = vData;
...
function resolveFileVersion(file, semanticVersions) {
 ...
 */
export function performSemanticSearch(query, ctx) {
    if (!semanticClient)
        return;
    ...
    if (ctx.activeVersionFilters.size > 0) {
        const semanticVersions = ctx.globalVersionColors.__semanticVersions || [];
    ...
    filteredResults.forEach((result, index) => {
        ...
        link.addEventListener('mouseenter', () => {
            ...
            ctx.updateSelection(ctx.searchResults.querySelectorAll('.search-result-item'));
        });
    });
}
...
function cleanFileToTitle(file) {
 ...
 */
function buildSnippetFragment(text, query) {
